...
| production | 本番 | `https://booking-api.stagehubs.net` | Coolify の自動デプロイ（`main` へのマージ） |
...
| `PUBLIC_BASE_URL` | 公開 URL（キャンセル URL・メールのリンク・OAuth のリダイレクト URI 生成） | `https://booking-api.stagehubs.net` |
| `ALLOWED_HOSTS` | Host ヘッダ検証（カンマ区切り。未設定なら検証しない） | `booking-api.stagehubs.net` |
...
3. **認証情報 → OAuth 2.0 クライアント ID** を種別「**ウェブ アプリケーション**」で作る
   - 承認済みのリダイレクト URI に
     `https://booking-api.stagehubs.net/v1/admin/google/oauth/callback` を登録する
...
```bash
curl -X POST https://booking-api.stagehubs.net/v1/admin/booking-types \
...
1. `https://booking-api.stagehubs.net/v1/admin/google/setup` を開く
...
curl -X POST https://booking-api.stagehubs.net/v1/admin/google/oauth/url \
  -H "X-Admin-Key: $ADMIN_API_KEY"
...
5. デプロイ後の確認
   ```bash
   curl -s https://booking-api.stagehubs.net/health   # {"status":"ok","version":"..."}
   curl -s https://booking-api.stagehubs.net/ready    # {"status":"ready","database":"ok"}
   ```
