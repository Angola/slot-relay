...
- 開発用 seed の受付時間を平日 **10:00〜18:00 から 12:00〜19:00** に変更した。
  60 分枠なので 12:00 / 13:00 / … / 18:00 の 7 枠になる。
  終了を 19:00 にしているのは、枠の終わりが受付終了時刻を超えない仕様のため
  （`AvailabilityCalculator#slots_in_window`）。18:00 開始の枠を出すには 19:00 が必要で、
  18:00 にすると 17:00 開始が最後の 6 枠になる。
  本番・ステージングの受付時間は DB の値なので、管理 API から別途変更する。
